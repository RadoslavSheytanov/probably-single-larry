...
├── .env.production              # VITE_WORKER_URL + VITE_LICENSE_SALT
├── public/icons/
└── src/
...
- Phase 6: Licensing rewritten to Cloudflare Worker + localStorage HMAC (zero server cost)
  ...
  - src/hooks/useHeartbeat.ts: deleted (not needed)
  - AppSettings: cleaned up (no activationTaps, licenseKey, licenseEmail)
  - Typechecks: 0 errors
